  ...
  ngOnInit() : void{
    ...
    this.apiTwitchService.validateToken(this.access_token).pipe(
      switchMap((data:any) => {
        ...
        return this.apiTwitchService.GetFollowedChannels(this.access_token, data.client_id, data.user_id);
      })).subscribe((data : any) => {
      this.followed_channels = data.data;
      console.log(this.followed_channels.length);
      this.followed_channels.forEach((channel: { broadcaster_login: string; is_live: string; profile_image_url : string;})  => {
        this.apiTwitchService.GetLiveStreams(this.access_token, this.client_id, channel.broadcaster_login).subscribe((data : any) => {
          if (data.data.length > 0){
            channel.is_live = "🔴";
          }else{
            channel.is_live = "";
          }
          this.apiTwitchService.GetUserInfos(this.access_token, this.client_id, channel.broadcaster_login).subscribe((data : any) => {
